...
const COMMANDS: Record<string, () => string[]> = {
  ...
  reading: () => [
    ...
    "",
    "Currently Reading:",
    ...DATA.reading.currentlyReading.map((item) => `  ${item.cover} ${item.title} - ${item.author}`),
    "",
    "Recently Completed:",
    ...DATA.reading.completed.map((item) => `  ${item.cover} ${item.title}`),
    "",
    "Scroll down to see the full reading section!",
  ],
  clear: () => [],
  "sudo hire mike": () => [
    "",
